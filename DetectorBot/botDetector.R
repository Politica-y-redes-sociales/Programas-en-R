...
carpeta = "/Users/alfonsoopazom/Desktop/Observatorio/ProgramasenR/DetectorBot"
...
while(i <= numArchivos)
{
  ...
  cuenta <- sqldf("SELECT Cuentas, numeros, length(Cuentas) Cantidad, length(numeros) Cantidad_num, 
                   round(((cast(length(numeros) as float))/(cast(length(Cuentas) as float)))*100,2) Porcentaje
                   FROM cuenta
  ...
  descripcion <- sqldf("SELECT DISTINCT(screen_name) cuenta, description Descripcion
                       FROM consulta WHERE Descripcion =''")
  ...
  write.csv(tabla_final$cuenta, paste(CarpetaResultados, 'Cuentas.csv', sep = "/"), row.names = FALSE)
