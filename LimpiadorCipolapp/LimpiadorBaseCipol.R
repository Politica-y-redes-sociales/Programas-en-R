...
carpeta<-"C:/Users/interbarometro/Desktop/ProgramasR/LimpiadorCipolapp"
...
for(i in 1:length(nombres[,1])){
  ...
  respuesta <- sqldf('SELECT DISTINCT(user_id), status_id, created_at, screen_name,text,source,display_text_width,
                      ...
                      media_url,"media_t.co" "media_t.co",media_expanded_url,media_type,ext_media_url,"ext_media_t.co" "ext_media_t.co",
                      ext_media_expanded_url,ext_media_type,mentions_user_id,mentions_screen_name,lang,quoted_status_id,quoted_text,
                      quoted_created_at,quoted_source,quoted_favorite_count,quoted_retweet_count,quoted_user_id,quoted_screen_name,quoted_name,
  ...
  write.csv(respuesta,file = paste(carpeta,"Resultados",nombres$nombres[i],sep = "/"),row.names = FALSE)
}
